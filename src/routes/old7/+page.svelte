<script>
  ...
  function on_key_up(letter) {
    ...
    //This is code check if the key pressed is the first letter of the array and if it is, it removes it from the array
      ...
      started = false;
      scores = [...scores, {time: timer, now: now.getTime(), speed:speed_text()}];
  ...
  function on_key_down(event){
    ...
    if(!event.key) return;
    if (event.key === 'Unidentified') return;//on mobile devices, the key is identified as Unidentified
  ...
  function speed_text() {
  speedText = guessesText.length / timer;
  return speedText.toFixed(4); // Returns speed with 4 decimal places
}
    ...
    <Grid>
        <Card col={12}>
          <div class="container">
            ...
            <div class="timer">Time: {timer}ms
              {#if timer > 0}
              <div class="speed">Speed: {speed_text()} letters/ms</div>
            {/if}
            </div>
            <div class="scores">
              <h2>High Scores</h2>
              <ul>
                {#each scores as score}
                  <li>
                    <span>Score: {score.time}ms at {new Date(score.now).toLocaleString()} Speed:{score.speed} letters/ms</span>
